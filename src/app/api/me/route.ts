...
export async function GET() {
  ...
  const [profileRes, faucetRes] = await Promise.all([
    supabase
      .from("profiles")
      .select("email_verified_at, public_id, referral_code, created_at, hilo_bet_count")
      .eq("id", user.id)
      .single(),
  ...
  const stats = {
    betCount: Number(profileRes.data?.hilo_bet_count ?? 0),
